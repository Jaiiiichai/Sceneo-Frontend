...
function PackageCheckoutContent() {
  ...
  const checkoutSubtotal = Number(studioPackage?.package_price || 0);
    ...
    <main className="min-h-screen bg-[#e5e7eb] px-4 pb-16 pt-4 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-6xl gap-5 lg:grid-cols-[1fr_380px]">
        ...
        <aside className="h-fit rounded-lg bg-slate-950 p-6 text-white shadow-xl lg:sticky lg:top-24">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-300">{studioPackage.makeup_available ? `${studioPackage.audience_key === 'couple' ? 'Couple' : 'Solo'} Make-up Bundle ${studioPackage.bundle_code === 'C' ? 'A' : 'B'}` : `${studioPackage.audience_name} · Bundle ${studioPackage.bundle_code}`}</p>
          ...
          <div className="space-y-3 text-sm text-slate-300">
            ...
            <p className="flex gap-2"><Camera size={17} /> {studioPackage.photography_minutes > 0 ? `${studioPackage.photography_minutes} mins with in-house pro photographer` : 'No in-house photographer included'}</p>
            <p className="flex gap-2"><Images size={17} /> {studioPackage.edited_photos > 0 ? `${studioPackage.edited_photos} edited photos` : 'No edited photos included'}</p>
            {studioPackage.makeup_available && <p className="flex gap-2"><CheckCircle2 size={17} /> Soft glam or full glam make-up with hairstyling{studioPackage.audience_key === 'couple' ? ' for male and female' : ''}</p>}
            <p className="flex gap-2"><CheckCircle2 size={17} /> Unlimited self-photography during your booking time</p>
